...
void CodeGenerator::visit(Program &node) {
    ...
    generateLine("#include <type_traits>");
    generateLine("");
    
    generateLine("template<typename T>");
    generateLine("void jadeSend(const T& data, int dest, int tag) {");
    indentLevel++;
    generateLine("if constexpr (std::is_same_v<T, int>) {");
    generateLine("    MPI_Send(&data, 1, MPI_INT, dest, tag, MPI_COMM_WORLD);");
    generateLine("} else if constexpr (std::is_same_v<T, float>) {");
    generateLine("    MPI_Send(&data, 1, MPI_FLOAT, dest, tag, MPI_COMM_WORLD);");
    generateLine("} else if constexpr (std::is_same_v<T, bool>) {");
    generateLine("    int val = data ? 1 : 0;");
    generateLine("    MPI_Send(&val, 1, MPI_INT, dest, tag, MPI_COMM_WORLD);");
    generateLine("} else if constexpr (std::is_same_v<T, std::string>) {");
    generateLine("    int len = data.size();");
    generateLine("    MPI_Send(&len, 1, MPI_INT, dest, tag, MPI_COMM_WORLD);");
    generateLine("    if(len > 0) MPI_Send(data.data(), len, MPI_CHAR, dest, tag, MPI_COMM_WORLD);");
    generateLine("} else {");
    generateLine("    int size = data.size();");
    generateLine("    MPI_Send(&size, 1, MPI_INT, dest, tag, MPI_COMM_WORLD);");
    generateLine("    if constexpr (std::is_same_v<typename T::value_type, int>) {");
    generateLine("        if(size > 0) MPI_Send(data.data(), size, MPI_INT, dest, tag, MPI_COMM_WORLD);");
    generateLine("    } else if constexpr (std::is_same_v<typename T::value_type, float>) {");
    generateLine("        if(size > 0) MPI_Send(data.data(), size, MPI_FLOAT, dest, tag, MPI_COMM_WORLD);");
    generateLine("    } else {");
    generateLine("        for(const auto& item : data) jadeSend(item, dest, tag);");
    generateLine("    }");
    generateLine("}");
    indentLevel--;
    generateLine("}");
    ...
    generateLine("template<typename T>");
    generateLine("void jadeRecv(T& data, int src, int tag) {");
    indentLevel++;
    generateLine("MPI_Status status;");
    generateLine("if constexpr (std::is_same_v<T, int>) {");
    generateLine("    MPI_Recv(&data, 1, MPI_INT, src, tag, MPI_COMM_WORLD, &status);");
    generateLine("} else if constexpr (std::is_same_v<T, float>) {");
    generateLine("    MPI_Recv(&data, 1, MPI_FLOAT, src, tag, MPI_COMM_WORLD, &status);");
    generateLine("} else if constexpr (std::is_same_v<T, bool>) {");
    generateLine("    int val;");
    generateLine("    MPI_Recv(&val, 1, MPI_INT, src, tag, MPI_COMM_WORLD, &status);");
    generateLine("    data = (val != 0);");
    generateLine("} else if constexpr (std::is_same_v<T, std::string>) {");
    generateLine("    int len;");
    generateLine("    MPI_Recv(&len, 1, MPI_INT, src, tag, MPI_COMM_WORLD, &status);");
    generateLine("    data.resize(len);");
    generateLine("    if(len > 0) MPI_Recv(&data[0], len, MPI_CHAR, src, tag, MPI_COMM_WORLD, &status);");
    generateLine("} else {");
    generateLine("    int size;");
    generateLine("    MPI_Recv(&size, 1, MPI_INT, src, tag, MPI_COMM_WORLD, &status);");
    generateLine("    data.resize(size);");
    generateLine("    if constexpr (std::is_same_v<typename T::value_type, int>) {");
    generateLine("        if(size > 0) MPI_Recv(data.data(), size, MPI_INT, src, tag, MPI_COMM_WORLD, &status);");
    generateLine("    } else if constexpr (std::is_same_v<typename T::value_type, float>) {");
    generateLine("        if(size > 0) MPI_Recv(data.data(), size, MPI_FLOAT, src, tag, MPI_COMM_WORLD, &status);");
    generateLine("    } else {");
    generateLine("        for(auto& item : data) jadeRecv(item, src, tag);");
    generateLine("    }");
    generateLine("}");
    indentLevel--;
    generateLine("}");
